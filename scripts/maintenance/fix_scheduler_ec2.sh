#!/bin/bash
# 快速修复EC2上的scheduler_daemon.py路径问题

echo "🔧 Fixing scheduler_daemon.py on EC2..."

cd ~/scraper || exit 1

# 备份旧文件
cp scheduler_daemon.py scheduler_daemon.py.backup

# 创建修复后的文件
cat > scheduler_daemon.py << 'ENDFILE'
#!/usr/bin/env python3
"""
...
echo "✅ scheduler_daemon.py has been fixed!"
echo ""
...
echo ""
echo "🎉 Done! Check if both services are running."
